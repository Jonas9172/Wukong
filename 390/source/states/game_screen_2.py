...
class GameScreen:
    ...
    def __init__(self):
        ...
        self.current_time = 0
        self.timer = 0
        self.info = info.Info('game_screen_2')
        self.next = 'results'
    ...
    def update_level_state(self):
        ...
        if self.pass_level == 1 and self.in_portal:
            self.level = level1.Level1(mode="online")
            self.level.initial_position(self.player)
            self.level.initial_position(self.player2)
            self.setup_background()
            self.on_ground = True
        ...
        if self.pass_level == 2 and self.in_portal:
            self.level = level2.Level2(mode="online")
            self.level.initial_position(self.player)
            self.in_portal = False

        if self.pass_level == 3 and self.in_portal:
            self.level = level3.Level3(mode="online")
            self.level.initial_position(self.player)
            self.setup_background()
            self.in_portal = False
    ...
    def setup_player(self):
        ...
        self.player2 = player.Player('wukong2_pixel')
    ...
    def update(self, surface, keys, timer, coin_number, result, net=None):
        if self.a == 0:
            self.net = net
            net.get_id()
            self.a = 1
        ...
        data = self.send_data()
        self.player2.rect.x, self.player2.rect.y = self.parse_data_pos(data)
        self.player2.as_rival(self.parse_data_image(data)[0], self.parse_data_image(data)[1])
        ...
        self.draw(self.parse_data_level(data))

        if not self.pass_level == 3 and not self.pass_level == 4:
            self.current_time = pygame.time.get_ticks()
            if self.current_time - self.timer >= 1200:
                self.reply_r = self.random()

                self.level.update(random_n=self.reply_r)
            ...
            if self.next_cloud or self.first_cloud:
                ...
                self.reply_r = self.random()
                self.level.update(self.game_window, random_n=self.reply_r)
                self.next_cloud = False
    ...
    def draw(self, level):
        ...
        if self.pass_level == level:
            self.game_ground.blit(self.player2.image, self.player2.rect)
    ...
    def send_data(self):
        ...
        if self.player.face_right:
            face = 1
        if not self.player.face_right:
            face = 0
        data = str(self.net.id) + ":" + str(self.player.rect.x) + "," + str(self.player.rect.y) + "," + str(self.player.frame_index) + "," + str(face) + "," + str(self.pass_level)
        reply = self.net.send(data)
        return reply

    def random(self):
        if self.pass_level == 3:
            random_request = str(self.net.id) + ":cpl3"
            reply = self.net.send(random_request)
            return reply
        if self.pass_level == 1 or self.pass_level == 2:
            random_request = str(self.net.id) + ":cpl1"
            reply = self.net.send(random_request)
            return reply

    @staticmethod
    def parse_data_pos(data):
        try:
